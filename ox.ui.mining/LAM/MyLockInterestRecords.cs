...
namespace OX.UI.LAM
{
    public partial class MyLockInterestRecords : DarkToolWindow, INotecaseTrigger, IModuleComponent
    {
        ...
        void DoReload()
        {
            ...
            {
                var ps = bizPlugin.GetAll<SelfLockKey, LockMiningRecordMerge>(InvestBizPersistencePrefixes.LockMiningInterestRecords);
                    ...
                    foreach (var g in ps.Where(m => this.Operater.Wallet.ContainsAndHeld(m.Key.Holder)).GroupBy(m => m.Value.AssetId))
                    {
                        ...
                        this.DoInvoke(() =>
                        {
                            ...
                            foreach (var p in g.OrderByDescending(m => m.Value.Timestamp))
                            {
                                ...
                                var subNode2 = new DarkTreeNode(UIHelper.LocalString($"出息额:{output.Value}", $"Profit Volume:{output.Value}"));
                                subNode2.NodeType = 2;
                                subNode2.Tag = p.Value;
                                subNode.Nodes.Add(subNode2);
                                var subsubNode = new DarkTreeNode(UIHelper.LocalString($"出息时间:{p.Value.Timestamp.ToDateTime().ToString("yyyy-MM-dd HH:mm:ss")}", $"Profit Time:{p.Value.Timestamp.ToDateTime().ToString("yyyy-MM-dd HH:mm:ss")}"));
                                subsubNode.NodeType = 1;
                                subsubNode.Tag = g.Key;
                                subNode2.Nodes.Add(subsubNode);

                                subsubNode = new DarkTreeNode(UIHelper.LocalString($"出息地址:{output.ScriptHash.ToAddress()}", $"Profit Address:{output.ScriptHash.ToAddress()}"));
