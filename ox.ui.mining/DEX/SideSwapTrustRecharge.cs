...
{
    public partial class SideSwapTrustRecharge : DarkDialog
    {
        ...
        public SideSwapTrustRecharge(INotecase operater, SlotSideTransaction sideTx) : this()
        {
        ...
        private void PayToDialog_Load(object sender, EventArgs e)
        {
            ...
            if (this.Operater.Wallet is OpenWallet openWallet)
            {
                ...
                {
                    if (act.Value.Targets.Contains(this.PoolAddress))
                    {
                    ...
                    else if (act.Value.SideScopes.IsNotNullAndEmpty())
                    {
                        foreach (var t in act.Value.SideScopes)
                        {
                            ...
                            {
                                if (ssl.SideStateList.Select(m => m.SideScriptHash).Contains(this.PoolAddress))
                                {
                                    atcts[act.Key] = act.Value;
                                    break;
                                }
                ...
                using (ScriptBuilder sb = new ScriptBuilder())
                {
                    ...
                    if (account != null)
                    {
                        ...
                        if (utxos.SortSearch(amount.GetInternalValue(), excludedUtxoKeys, out AssetTrustUTXO[] selectedUtxos, out long remainder))
                        {
                            ...
                            outputs.Add(new TransactionOutput { AssetId = assetId, Value = amount, ScriptHash = this.PoolAddress });
                            if (remainder > 0)
                            {
